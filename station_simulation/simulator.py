...
def run_sim(
    ...
    for t in range(config.T):
        ...
        results["infected"] += [
            infex.get_nb_working(t) - infex.get_nb_working_spreading(t)
        ]
...
def get_performance(usetest):
    ...
    for k in range(config.Nsim):
        ...
        if config.plotting:
            if k > 0:
                ...
                alpha = 1.0
                ax.plot(
                    results["uninfected"], "g-", label="working uninfected", alpha=alpha
                )
                ax.plot(
                    results["infected"],
                    "m-",
                    label="working infected non-spreading",
                    alpha=alpha,
                )
                ax.plot(
                    results["spreading"],
                    "r-",
                    label="working infected spreading",
                    alpha=alpha,
                )
                ax.plot(
                    results["immune"],
                    "b-",
                    label="working recovered immune",
                    alpha=alpha,
                )
        ...
        if config.plotting:
            axp1.plot(
                [died_frac * 100], [minwork_frac * 100], "r", marker="x", alpha=1.0
            )
    ...
    if config.output_summary:
        ...
        print(f"avg maximal tests/day= {np.average(overall_tests_perday_max):7.2f}")
    ...
    return {
        ...
        "seq_seq_avg_working": np.average(
            seq_seq_working, axis=0
        ),  # point-wise average
    }

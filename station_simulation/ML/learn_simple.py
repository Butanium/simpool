...
def swap_agents(i, j, graphs, *fields):
    ...
    for t in range(current_T):
        ...
        for k in graphs[t].keys():
            graphs[t][k] = {
                (i if out_neigh == j else (j if out_neigh == i else out_neigh)): value
    ...
    def generate_data(self):
        ...
        if random.random() < 0.5:
            ...
            spreading_list = [
                ...
                if sum(spreading[t][j] for t in range(config.T)) > 0
            ]
            if spreading_list == []:
...
def create_NN():
    ...
    model.add(
        LSTM(128, name="hidden", return_sequences=False, dropout=0.2, activation="tanh")
    )

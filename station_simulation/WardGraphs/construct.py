...
def plot_degree_hist(G):
    ...
    for t in col.keys():
        ...
        degree_sequence = sorted(
            [nd[1] for nd in node_degree_sequence if agents[nd[0]] == t], reverse=True
        )
...
def plot_graph(G, agents):
    ...
    nx.draw_networkx(
        G, with_labels=True, node_size=200, node_color=node_color, cmap=cmap
    )
    plot_degree_hist(G)
...
def frequent_contacts(myid, graphs, agents, threshold=3, toCat=None):
    ...
    for perid in graphs[0].nodes:
        cnt = sum(
            [
                1 if perid in G.neighbors(myid) and agents[perid] in toCats else 0
                for G in graphs
            ]
        )
        if cnt >= threshold:
...
def change_relativeto_kernel(myid, graphs, threshold=3):
    ...
    for G in graphs:
        try:
            rel_G_minus_Freq = len(set(G.neighbors(myid)) - set(freq_contacts)) / len(
                freq_contacts
            )
            rel_Freq_minus_G = len(set(freq_contacts) - set(G.neighbors(myid))) / len(
                freq_contacts
            )
...
def union_nb_contacts(graphs, agents, fromCat=None, toCat=None):
    ...
    return [
        len([otherid for otherid in G.neighbors(myid) if agents[otherid] in toCats])
        for G in graphs
        for myid in graphs[0].nodes
        if agents[myid] in fromCats
    ]
...
def average_min_contacts(myid, graphs):
    return sum(
        [
            G[myid][otherid]["weight"] / 3
            for G in graphs
            for otherid in G.neighbors(myid)
        ]
    ) / len(graphs)
...
if __name__ == "__main__":
    ...
    for myid in agents.keys():
        print(
            f"{myid} [{agents[myid]}] kernelsize = {len(frequent_contacts(myid, graphs, agents))}"
        )
    ...
    for cat1 in col.keys():
        ...
        for cat2 in col.keys():
            data_full = [
                len(frequent_contacts(myid, graphs, agents, toCat=cat2))
                for myid in agents.keys()
                if agents[myid] == cat1
            ]
            ...
            if len(data) > 0:
                ...
                print(
                    f"   Category.{cat2}: lambda: nbinom.rvs(**{best_params(data, plot=False)}),"
                )

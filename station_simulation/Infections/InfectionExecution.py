...
class InfectionExecution(object):
    def __init__(self, initial_infections={"T_range": (0, 10), "N_range": (0, 1)}):
        ...
        """
        self.infections = {"internal": [], "external": []}
        self.contacted = dict()
        ...
        for i in myids:
            t = int(
                random.uniform(
                    initial_infections["T_range"][0], initial_infections["T_range"][1]
                )
            )
    ...
    def get_state_vec(self):
        state = {agent_id: agent.state for agent_id, agent in self.agents.items()}
        return [state]

    def get_contacts_attime(self, agent_id, t, duration_threshold=config.DURATION_TH):
        if agent_id in self.contacted.keys():
            contacts_at_time = [
                c["contacts"] for c in self.contacted[agent_id] if c["time"] == t
            ]
            assert len(contacts_at_time) <= 1
            if contacts_at_time:
                thresholded_contacts = [
                    other
                    for other in contacts_at_time[0].keys()
                    if contacts_at_time[0][other] >= duration_threshold
                ]
    ...
    def get_contacts_attime_withduration(self, agent_id, t):
        if agent_id in self.contacted.keys():
            ...
            assert len(contacts_at_time) <= 1
            if contacts_at_time:
                contacts_withduration = {
                    other: contacts_at_time[0][other]
                    for other in contacts_at_time[0].keys()
                }
    ...
    def get_contacts_fromtime(
        self, agent_id, t_from, duration_threshold=config.DURATION_TH
    ):
        ret = set()
    ...
    def get_nb_working(self, t):
        return len(
            [
                i
                for i, state in self.states[t].items()
                if state["working"] and state["staff"]
            ]
        )
    ...
    def get_nb_quarantined(self, t):
        return len(
            [
                i
                for i, state in self.states[t].items()
                if state["quarantined"] and state["staff"]
            ]
        )
    ...
    def get_nb_working_spreading(self, t):
        return len(
            [
                i
                for i, state in self.states[t].items()
                if state["working"] and state["spreading"] and state["staff"]
            ]
        )
    ...
    def get_nb_sick_leave(self, t):
        return len(
            [
                i
                for i, state in self.states[t].items()
                if state["sick_leave"] and state["staff"]
            ]
        )
    ...
    def get_nb_infected(self, t):
        return len(
            [
                i
                for i, state in self.states[t].items()
                if state["infected"] and state["staff"]
            ]
        )
    ...
    def get_nb_working_immune(self, t):
        return len(
            [
                i
                for i, state in self.states[t].items()
                if state["working"] and state["infected"] and state["staff"]
            ]
        )
    ...
    def get_infections_per_infected(self):
        ...
        for myid in was_infecting:
            li = list(filter(lambda inf: inf[2] == myid, self.infections["internal"]))
                        ...
                        for so in today_contacts.keys():
                            ...
                            if self.agents[so].spreading:
                                ...
                                if u < config.pinf(duration):
                                    self.log_internal_infection(
                                        self.agents[so].time_infected, self.t, so, i
                                    )

...
class ExecutionPlotter(object):
    ...
    def __get_firstdayofspreading__(self, agent_id) -> float:
        spreading_days = [
            i for i, s in enumerate(self.agents_states[agent_id]) if s["rna"] > 0
        ]
    ...
    def __draw_infected_events__(self):
        for line, agent_id in enumerate(self.agents_order):
            infected_days = [
                i for i, s in enumerate(self.agents_states[agent_id]) if s["infected"]
            ]
    ...
    def __draw_symptoms_events__(self):
        for line, agent_id in enumerate(self.agents_order):
            mild_days = [
                i
                for i, s in enumerate(self.agents_states[agent_id])
                if s["symptoms"] is Symptoms.MILD
            ]
            severe_days = [
                i
                for i, s in enumerate(self.agents_states[agent_id])
                if s["symptoms"] is Symptoms.SEVERE
            ]
    ...
    def add_agent(self, infex: InfectionExecution, agent_id: int):
        self.agents_states[agent_id] = [
            infex.states[t][agent_id] for t in range(self.T)
        ]
        self.agents_order += [agent_id]

    def sort(self):
        self.agents_order.sort(
            key=lambda i: self.__get_firstdayofspreading__(agent_id=i)
        )
